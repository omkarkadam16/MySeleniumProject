	...
	def test_master(self):
		...
		if self.switch_frames("LinkId-select"):
			...
			for index, row in df_sheet1.iterrows():
				try:
					print(f"Processing AsmQualifiedName (Sheet1): {row['AsmQualifiedName']}")
					...
					print(f"Failed to process AsmQualifiedName {row['AsmQualifiedName']} (Sheet1): {str(e)}")
			...
			for index, row in df_sheet2.iterrows():
				try:
					...
					if pd.notna(row['Id For Selection Criteria']):
						self.send_keys(By.ID, "IdSelectionCriteria", row['Id For Selection Criteria'])

					if pd.notna(row['Import Column Name']):
						self.send_keys(By.ID, "ImportColumnName", row['Import Column Name'])

					if pd.notna(row['Foreign Entity Name']):
						self.send_keys(By.ID, "ForeignEntityName", row['Foreign Entity Name'])
					...
					if pd.notna(row['Foreign Display Name']):
						self.send_keys(By.ID, "ForeignDisplayName", row['Foreign Display Name'])
					...
					if self.switch_frames("btnSave-ImportEntityColumnMapSessionName782"):
						...
						print(f"Failed to switch to the correct frame for {row['Entity Code']}")

				except Exception as e:
					print(f"Failed to process Entity Code {row['Entity Code']} (Sheet2): {str(e)}")

			# Save updated Sheet2 back to Excel
			df_sheet2.to_excel("JV(Cash & Bank).xlsx", index=False, engine="openpyxl")

			print("✅ JV(Cash & Bank) Sheet2 Completed! Check JV(Cash & Bank).xlsx for results.")
